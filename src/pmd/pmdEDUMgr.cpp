...
int pmdEDUMgr::postEDUPost(EDUID eduID, pmdEDUEventTypes type, bool release , void *pData) {
   ...
   _mutex.get_shared();
   if ((it = _runQueue.find(eduID)) == _runQueue.end()) {
      ...
      if ((it = _idleQueue.find(eduID)) == _idleQueue.end()) {
...
int pmdEDUMgr::returnEDU(EDUID eduID, bool force, bool* destroyed) {
   ...
   if (!isPoolable(type) || force || isDestroyed() || size() >(unsigned int)pmdGetKRCB()->getMaxPool()) {
      ...
      if (destroyed) {
         // when we try to pool the EDU, destroyed set to true only when the EDU can't be found in the list
         if (rc == EDB_SYS)
            *destroyed = true;
         else
            *destroyed = false;
...
int pmdEDUMgr::startEDU(EDU_TYPES type, void* arg, EDUID *eduid) {
   ...
   if (_idleQueue.empty() == true || !isPoolable(type)) {
      // note that EDU types other than "agent" shouldn't be pooled at all release latch before calling createNewEDU
      _mutex.release();
   ...
   for(it = _idleQueue.begin();(it != _idleQueue.end()) &&((*it).second->getStatus() != PMD_EDU_IDLE); it++);

   // if everything in idleQueue are in DESTROY status, we still need to
   // create a new EDU
   if (it == _idleQueue.end()) {
...
int pmdEDUMgr::_createNewEDU(EDU_TYPES type, void* arg, EDUID *eduid) {
   ...
   if (_runQueue.find(_EDUID) != _runQueue.end()) {
      _mutex.release();
      rc = EDB_SYS;
      probe = 10;
      goto error;
   ...
   if (_idleQueue.find(_EDUID) != _idleQueue.end()) {
      _mutex.release();
      rc = EDB_SYS;
      probe = 15;
      goto error;
...
pmdEDUCB *pmdEDUMgr::getEDU(unsigned int tid) {
   ...
   if (it == _tid_eduid_map.end()) {
      pResult = NULL;
      goto done;
   }
   eduid =(*it).second;
   it1 = _runQueue.find(eduid);
   if (it1 != _runQueue.end()) {
      pResult =(*it1).second;
      goto done;
   }
   it1 = _idleQueue.find(eduid);
   if (it1 != _idleQueue.end()) {
      pResult =(*it1).second;
      goto done;
   }
done :
