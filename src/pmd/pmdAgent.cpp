...
int pmdAgentEntryPoint(pmdEDUCB *cb, void *arg) {
    ...
    while(!disconnect) {
        ...
        if (packetLength+1 > receiveBufferSize) {
            PD_LOG(PDDEBUG, "Receive buffer size is too small: %d vs %d, increasing...", receiveBufferSize, packetLength );
        ...
        if ((rc = eduMgr->activateEDU(myEDUID)) != EDB_OK) {
            goto error;
        }
        ...
        if ((rc = eduMgr->waitEDU(myEDUID)) != EDB_OK) {
            goto error;
        }
    }
done:
...
error:
    switch (rc) {
        ...
        case EDB_EDU_INVAL_STATUS:
            ...
            break;
        case EDB_INVALIDARG:
            PD_LOG(PDSEVERE, "Invalid argument receieved by agent, probe %d", probe);
            break;
        case EDB_OOM :
