...
int dmsFile::initialize (const char *pFileName) {
   ...
   PD_RC_CHECK(rc, PDERROR, "Failed to open file %s, rc = %d", _pFileName, rc);
getfilesize:
    // get file size
...
int dmsFile::_extendFile (int size) {
    ...
    if (size % DMS_EXTEND_SIZE != 0) {
        rc = EDB_SYS;
        PD_LOG(PDERROR, "Invalid extend size, must be multiple of %d", DMS_EXTEND_SIZE);
        goto error;
    }
    // write file
        ...
        for (int i = 0; i < numSegments; i++) {
            ...
            for (unsigned int k = 0; k < DMS_PAGES_PER_SEGMENT; k++) {
                ...
                for (unsigned int s = 0; s < slotID; s++) {
                    slotOffset = *(SLOTOFF*)(data + k*DMS_PAGESIZE + sizeof(dmsPageHeader) + s*sizeof(SLOTID));
