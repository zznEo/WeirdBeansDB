...
public :
    ...
    */
   /*
    * This function must be called against a thread that either in creating/idle or waiting status
    * Threads in creating/waiting status should be sit in runqueue
    * Threads in idle status should be sit in idlequeue
    * This function set the status to PMD_END_RUNNING and bring the control block to runqueue if it was idle status
    * Parameter:
    *   EDU ID (UINt64)
    * Return:
    *   SDB_OK (success)
   ...
   /*
    * This function must be called against a thread that in running status
    * Threads in running status will be put in PMD_EDU_WAITING and remain in runqueue
    * Parameter:
    *   EDU ID (UINt64)
    * Return:
    *   SDB_OK (success)
    ...
    * Depends on if there's any idle EDU, manager may choose an existing idle thread or creating a new threads to serve the request
    * Parmaeter:
    ...
    * If the EDU is doing some other activity at the moment, it may not able to consume the event right away
    ...
    * If there are more than one event sitting in the queue  waitEDU function should pick up the earliest event
   ...
   int waitEDUPost(EDUID eduID, pmdEDUEvent& event, long long millsecond);
    ...
    /*
    * This function should return an waiting/creating EDU to pool (cannot be running)
    * Pool will decide whether to destroy it or pool the EDU
    * Any thread main function should detect the destroyed output deactivateEDU supposed only happened to AGENT EDUs
    * Parameter:
    *   EDU ID (EDUID)
    * Output:
