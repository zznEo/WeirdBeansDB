...
class pmdEDUEvent {
public:
    pmdEDUEvent(): _eventType(PMD_EDU_EVENT_NONE), _relase(false), _Data(NULL){}
    pmdEDUEvent(pmdEDUEventTypes type): _eventType(type), _relase(false), _Data(NULL){}
    pmdEDUEvent (pmdEDUEventTypes type, bool relase, void *data): _eventType(type), _relase(relase), _Data(data){}
    void reset() {
